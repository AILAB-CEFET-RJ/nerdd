...
# parser = argparse.ArgumentParser(description="Diferença dos dados com objeto ou sem")

# Adicionando um parâmetro obrigatório
# parser.add_argument("parametro", type=str, help="Parâmetro de entrada")

# Lendo os argumentos
# args = parser.parse_args()

# Utilizando o parâmetro no código
# print(f"Parâmetro recebido: {args.parametro}")
...
def save_differences_to_csv(pred,label, output_file):
    with open(output_file, mode="w", newline="") as arquivo:
        escritor = csv.writer(arquivo)
        # Escreve o cabeçalho (opcional)
        escritor.writerow(["Predict", "Label"])
        # Escreve as linhas
        for item1, item2 in zip(pred,label):
