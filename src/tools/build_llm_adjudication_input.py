...
- input: ranked pseudolabel candidates JSONL produced from the baseline pipeline,
  already enriched with explicit `gliner2_entities`
- process: match baseline and GLiNER2 predictions conservatively
...
def build_summary(rows: list[dict], counters: Counter, args: argparse.Namespace) -> dict:
    ...
    return {
        ...
        "gliner2_source": "precomputed_entities",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input", required=True, help="Input JSON or JSONL with baseline candidate rows already enriched with gliner2_entities.")
    parser.add_argument("--output-jsonl", required=True, help="Output JSONL with matching metadata and review seeds.")
