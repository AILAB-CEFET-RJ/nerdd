...
class AuditRefitRegressionsTests(unittest.TestCase):
    def test_audit_detects_boundary_and_missing_regressions(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(summary["loss_reason_counts_by_label"]["Location"]["boundary_or_partial"], 1)
            self.assertEqual(summary["loss_reason_counts_by_label"]["Location"]["missing_entity"], 1)
    ...
    def test_audit_tracks_wrong_label_confusions(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            gold_path = root / "gold.json"
            baseline_path = root / "baseline.jsonl"
            ...
            gold_rows = [
                {
                    "text": "quartel da aeronáutica",
                    "spans": [{"start": 11, "end": 22, "label": "Organization"}],
                }
            ]
            gold_path.write_text(json.dumps(gold_rows, ensure_ascii=False), encoding="utf-8")
            ...
            baseline_rows = [
                {
                    "text": "quartel da aeronáutica",
                    "entities": [{"start": 11, "end": 22, "label": "Organization", "text": "aeronáutica"}],
                }
            ]
            baseline_path.write_text(json.dumps(baseline_rows[0], ensure_ascii=False) + "\n", encoding="utf-8")

            candidate_rows = [
                {
                    "text": "quartel da aeronáutica",
                    "entities": [{"start": 11, "end": 22, "label": "Location", "text": "aeronáutica"}],
                }
            ]
            candidate_path.write_text(json.dumps(candidate_rows[0], ensure_ascii=False) + "\n", encoding="utf-8")
            ...
            summary = json.loads((out_dir / "summary.json").read_text(encoding="utf-8"))
            self.assertEqual(summary["loss_reason_counts"]["wrong_label"], 1)
            self.assertEqual(summary["loss_reason_counts_by_label"]["Organization"]["wrong_label"], 1)
            self.assertEqual(summary["wrong_label_confusions"]["Organization->Location"], 1)
